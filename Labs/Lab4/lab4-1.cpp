...
class TicTacToe {
    ...
    public:
        ...
        void jogadaIA() {
            ...
            bool maximizingPlayer = jogador(tabuleiro) == Player::X;
            pair<int, pair<int, int>> bestMove = minimax(tabuleiro, INT_MAX, -INT_MAX, INT_MAX, maximizingPlayer);
...
class StrangeTicTacToe : public TicTacToe {
    ...
    public:
        ...
        void jogadaIA(){
            // Undo the last player move, find the worst move for the player and make it
            if(history.size() > 1){
                tabuleiro = history[history.size() - 2]; // Get the previous state
                ...
                bool maximizingPlayer = jogador(tabuleiro) == Player::X; //Player::X starts the game and is the maximizing player
                pair<int, pair<int, int>> bestMove = minimax(tabuleiro, INT_MAX, -INT_MAX, INT_MAX, !maximizingPlayer);
                tabuleiro = resultado(tabuleiro, bestMove.second);
                push_to_history(tabuleiro);

                // Now we need to find the best move for the AI and make it
                bestMove = minimax(tabuleiro, INT_MAX, -INT_MAX, INT_MAX, !maximizingPlayer);
                tabuleiro = resultado(tabuleiro, bestMove.second);
                push_to_history(tabuleiro);

            } else { // If there is no previous state, just play normally
                bool maximizingPlayer = jogador(tabuleiro) == Player::X; //Player::X starts the game and is the maximizing player
                pair<int, pair<int, int>> bestMove = minimax(tabuleiro, INT_MAX, -INT_MAX, INT_MAX, maximizingPlayer);
                tabuleiro = resultado(tabuleiro, bestMove.second);
                push_to_history(tabuleiro);
